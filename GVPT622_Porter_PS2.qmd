...
**Are we supposed to have data already? Where is it supposed to come from if so? If not, do you want code included in this?**

**No code/data, just writing the design - describe how we would collect the data**

**key element of experimental design - random sampling, groups with different names - don't miss one of the groups**

**follow whats done in the lab- include the fundamentals of an experiment and apply to individual us voters consume news. – look at polling companies**.

**2 parts - desgin the experiement, and hten also tell about any shortcomings (whats addressed ithe design or not). make sure to include both**
...
**1-2 sentences (yes or no)**

**Look in Readings for this week for sampling variability - 1k people that are all very different, large variability is different from the general population, but if repeated multiple times it decreases the sampling variability - just one sample isn't varaibility, but if repeated the samplling many times then how likely or different are they (sampling variability is due to how large your poualtion)**
...
B.  A sample of 75 randomly selected countries' educational funding levels for each of the past 25 years.
